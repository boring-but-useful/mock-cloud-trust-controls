...
GitHub Actions runs `make verify` for pull requests, pushes to `main`, a weekly UTC schedule, and manual dispatches. The scheduled run surfaces time-dependent exception warnings or failures even when the repository is otherwise quiet. A separate pull-request check rejects newly introduced dependencies with known moderate-or-higher vulnerabilities. Dependabot checks the pinned Python dependency and pinned GitHub Actions weekly.
